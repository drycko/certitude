...
use App\Http\Controllers\Tenant\PowerbiManager\PowerbiController;
...
Route::middleware([
    ...
    Route::prefix('/t')->middleware(['auth:tenant', 'must.change.password', 'subscription.check'])->group(function () {
        ...
        Route::get('/home', [DashboardController::class, 'index'])->name('tenant.home'); // Keep for compatibility
        Route::get('/profile', [DashboardController::class, 'profile'])->name('tenant.dashboard.profile');
        Route::post('/profile', [DashboardController::class, 'updateProfile'])->name('tenant.dashboard.profile.update');

        // knowledge base
        Route::get('/knowledge-base', [DashboardController::class, 'knowledgeBase'])->name('tenant.knowledge-base');

        Route::get('/help', [DashboardController::class, 'help'])->name('tenant.help');

        // notifications
        Route::get('/notifications', [DashboardController::class, 'notifications'])->name('tenant.notifications.index');
        Route::post('/notifications/mark-as-read', [DashboardController::class, 'notificationsMarkAsRead'])->name('tenant.notifications.mark-as-read');
        // mark all as read
        Route::post('/notifications/mark-all-as-read', [DashboardController::class, 'notificationsMarkAllAsRead'])->name('tenant.notifications.mark-all-as-read');

        // File routes
        Route::resource('files', FileController::class)->names([
            'index' => 'tenant.files.index',
            'create' => 'tenant.files.create',
            'store' => 'tenant.files.store',
            'show' => 'tenant.files.show',
            'edit' => 'tenant.files.edit',
            'update' => 'tenant.files.update',
            // 'destroy' => 'tenant.files.destroy',
        ]);
        Route::delete('/files/{file}', [FileController::class, 'destroy'])->name('tenant.files.destroy');
        Route::get('/files/management/bulk', [FileController::class, 'bulk'])->name('tenant.files.bulk');
        Route::post('/files/batch-download', [FileController::class, 'batchDownload'])->name('tenant.files.batch-download');
        Route::get('/files/{file}/download', [FileController::class, 'download'])->name('tenant.files.download');
        Route::get('/files/{file}/preview', [FileController::class, 'preview'])->name('tenant.files.preview');
        
        // Bulk operations (admin only)
        Route::middleware(['role:admin|super-user'])->group(function () {
            Route::post('/files/bulk-activate', [FileController::class, 'bulkActivate'])->name('tenant.files.bulk-activate');
            Route::post('/files/bulk-deactivate', [FileController::class, 'bulkDeactivate'])->name('tenant.files.bulk-deactivate');
            Route::post('/files/bulk-make-public', [FileController::class, 'bulkMakePublic'])->name('tenant.files.bulk-make-public');
            Route::post('/files/bulk-make-private', [FileController::class, 'bulkMakePrivate'])->name('tenant.files.bulk-make-private');
            Route::post('/files/bulk-set-expiry', [FileController::class, 'bulkSetExpiry'])->name('tenant.files.bulk-set-expiry');
            Route::post('/files/bulk-delete', [FileController::class, 'bulkDelete'])->name('tenant.files.bulk-delete');
        });
        ...
        Route::resource('powerbi-links', PowerbiController::class)->names([
            'index' => 'tenant.powerbi-links.index',
            'create' => 'tenant.powerbi-links.create',
            'store' => 'tenant.powerbi-links.store',
            'show' => 'tenant.powerbi-links.show',
            'edit' => 'tenant.powerbi-links.edit',
            'update' => 'tenant.powerbi-links.update',
            'destroy' => 'tenant.powerbi-links.destroy',
        ]);
        Route::get('/powerbi-links/{id}/view', [PowerbiController::class, 'view'])->name('tenant.powerbi-links.view');
        Route::get('/powerbi-links/{id}/iframe', [PowerbiController::class, 'iframe'])->name('tenant.powerbi-links.iframe');
        Route::get('/powerbi-links/{id}/get-embed-data', [PowerbiController::class, 'getEmbedData'])->name('tenant.powerbi-links.get-embed-data');
        Route::get('/powerbi-links/management/bulk', [PowerbiController::class, 'bulk'])->name('tenant.powerbi-links.bulk');
        // stop impersonation route
        Route::post('/users/stop-impersonation', [UserController::class, 'stopImpersonation'])->name('tenant.users.stop-impersonation');

        // Admin routes
        Route::middleware(['role:admin|super-user'])->group(function () {
            // search suggestion (with query param)
            Route::get('/users/search', [UserController::class, 'searchSuggestions'])->name('tenant.users.search');

            // Access requests admin UI (under users prefix) - must be declared before the users resource
            Route::get('/users/access-requests', [AccessRequestController::class, 'index'])->name('tenant.users.access-requests.index');
            Route::put('/users/access-requests/{accessRequest}/status', [AccessRequestController::class, 'updateStatus'])->name('tenant.users.access-requests.update-status');
            ...
            Route::post('/users/resend-welcome-email/{user}', [UserController::class, 'resendWelcomeEmail'])->name('tenant.users.resend-welcome-email');
            Route::post('/users/{user}/toggle-status', [UserController::class, 'toggleStatus'])->name('tenant.users.toggle-status');
            Route::post('/users/{user}/reset-password', [UserController::class, 'resetPassword'])->name('tenant.users.reset-password');
            Route::put('/users/{user}/update-password', [UserController::class, 'updatePassword'])->name('tenant.users.update-password');
            // impersonate user
            Route::post('/users/{user}/impersonate', [UserController::class, 'impersonate'])->name('tenant.users.impersonate');
            ...
            Route::prefix('master-data')->name('tenant.master-data.')->group(function () {
            ...
            Route::get('/company-logos/{filename}', [CompanyController::class, 'downloadLogo'])->name('tenant.company.logo.download');

            Route::prefix('trashed-data')->name('tenant.trashed-data.')->group(function () {
                ...
                Route::get('powerbi-links', [PowerbiController::class, 'trashed'])->name('powerbi-links.index');
                Route::post('powerbi-links/{id}/restore', [PowerbiController::class, 'restore'])->name('powerbi-links.restore');
                Route::post('powerbi-links/{id}/force-delete', [PowerbiController::class, 'forceDelete'])->name('powerbi-links.force-delete');
                Route::post('powerbi-links/bulk-restore', [PowerbiController::class, 'bulkRestore'])->name('powerbi-links.bulk-restore');
                Route::post('powerbi-links/bulk-force-delete', [PowerbiController::class, 'bulkForceDelete'])->name('powerbi-links.bulk-force-delete');
        ...
        // Fallback route should be at the end to catch any unmatched routes
