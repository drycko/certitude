...
class Document extends Model
{
    ...
    {
        return $this->belongsToMany(Fbo::class, 'document_fbo');
    }

    public function subDocumentType(): BelongsTo
    {
        return $this->belongsTo(DocumentType::class, 'sub_document_type_id');
    }

    public function commodities(): BelongsToMany
    {
        return $this->belongsToMany(Commodity::class, 'document_commodity');
    }

    public function varieties(): BelongsToMany
    {
        return $this->belongsToMany(Variety::class, 'document_varieties');
    }
    ...
    public function isExpiringSoon($days = 14): bool
    {
        ...
        // \Log::info('Checking if document is expiring soon', [
        //     'document_id' => $this->id,
    ...
    /**
     * Check if this document was imported from legacy system
     */
    public function isLegacyDocument(): bool
    {
    ...
    /**
     * Scope to filter legacy documents
     */
    ...
    /**
     * Scope to filter by sub document type
     */
    public function scopeBySubType($query, $subTypeId)
    {
        return $query->where('sub_document_type_id', $subTypeId);
    }

    /**
     * Document activity log (get activity that has table_name 'documents' and record_id = this document's id)
     */
    public function userActivities()
    {
        return UserActivity::where('table_name', 'documents')
            ->where('record_id', $this->id)->orderBy('created_at', 'desc')
            ->get();
    }

    /**
     * Document has user activities from users who are not admins and associated with grower(many growers relationship)
     * grower relationship is defined in User model, and we have to track users who are associated with grower_id in document metadata
     */
    ...
    {
        return $this->hasMany(UserActivity::class, 'record_id')->where('table_name', 'documents');
    }
    ...
    {
        $lastLog = UserActivity::where('table_name', 'documents')
            ->where('record_id', $this->id)
    ...
    /**
     * Check if document has been viewed by any grower user
     */
    ...
    /**
     * Get grower users who have viewed this document
     */
    public function getGrowerViewers()
    {
        ...
        $viewerIds = UserActivity::where('table_name', 'documents')
            ->where('record_id', $this->id)
            ->whereIn('activity_type', ['view', 'download'])
            ->whereIn('user_id', $growerUserIds)
            ->distinct()
    ...
    /**
     * Get distinct vessel names from metadata of all documents
     * @return array
     */
